...
The BuildBundles, RelinkLocalization, FixShaders (in sources), and localizationLogger.dll (in releases) downloaded.
...
4. In the "Editor" folder, drag and drop BuildBundles, RelinkLocalization, FixShaders, and your LogOutput.log (from LocalizationLogger.dll) files from your file system.
...
14. From the tools tab at the top, select "Fix Unassigned Shaders on Prefabs". This will make most prefabs no longer pink.
15. Modify to your hearts content, and proceed to the building section.

Instructions to build your custom hole:
1. From the tools tab at the top, select "Fix Unassigned Shaders in Open Scene". This must be done on EACH MAP (only once needed), and it only applies to the current open one. This will fix things such as trees and bushes from being pink.
2. From the tools tab at the top, select "Relink Localization". This must be done on EACH MAP (only once needed), and it only applies to the current open one. This will fix most of the text in levels.
...
8. Create a copy of the template json file and name it the same as the bundle, "{username}.{packname}.JSON". Fill out the information for your hole(s) and make sure the number of holes matches how many are in the bundle.
